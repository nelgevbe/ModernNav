...
- 每日壁纸 — 一键切换 Bing 每日一图，主题色随壁纸自动提取
- 1080p / 2K / 4K 视口自适应，所有尺寸按比例缩放
- 明暗主题切换（支持跟随系统），全局主题色换色
- 私密分类 — 标记为私密的分类仅登录状态可见，访客完全不可见
- 中英双语切换
- PWA 离线缓存
...
- 外观设置 — 背景图片、每日壁纸、主题色（支持从图片自动提取）、玻璃参数（模糊/饱和度/噪点/着色）、圆角缩放、导航栏样式
- 数据管理 — JSON 导入导出，浏览器书签 HTML 导入
- 使用统计 — 分类/链接概览、零点击统计、最常访问 Top 10
- 安全设置 — 修改管理密码
- 链接表单自动抓取网页标题和描述
...
- 关系化存储 — D1 分表 (categories / subcategories / links) + config KV，schema 版本自动迁移
