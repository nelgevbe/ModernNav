...
export const GlassCard: React.FC<GlassCardProps> = ({ 
  ...
  const MIN_TINT = isDark ? 0.20 : 0.3; 
  const MAX_TINT = isDark ? 0.80 : 0.8;
    ...
    >
      ...
      {hoverEffect && (
        <div className="absolute inset-0 pointer-events-none rounded-2xl overflow-hidden z-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
           <div className={`absolute top-0 left-0 w-[200%] h-full bg-gradient-to-r from-transparent ${isDark ? 'via-white/[0.05]' : 'via-white/20'} to-transparent -translate-x-full group-hover:animate-shimmer`} />
        </div>
      )}

      {/* LAYER 4: CONTENT */}
      <div className={`relative z-10 w-full h-full flex flex-col items-center justify-center pointer-events-none ${isDark ? 'text-white' : 'text-slate-800'}`}>
        {children}
      </div>
    </div>
  );
