...
export const SearchBar: React.FC<SearchBarProps> = ({ themeMode }) => {
  ...
  const containerStyle = {
    backdropFilter: query || isDropdownOpen || isFocused
      ? `blur(12px) saturate(160%)`
      : `blur(0px) saturate(100%)`,
    WebkitBackdropFilter: query || isDropdownOpen || isFocused
      ? `blur(12px) saturate(160%)`
      : `blur(0px) saturate(100%)`,
  ...
  const placeholderColor = isDark ? 'placeholder-white/20 focus-within:placeholder-white/50' : 'placeholder-slate-600/30 focus-within:placeholder-slate-600';
  const iconColor = isDark ? 'text-white/30 group-hover:text-white/80' : 'text-slate-500/40 group-hover:text-slate-700';
  const dividerColor = isDark ? 'border-r border-white/5 group-hover:border-white/10' : 'border-r border-slate-500/5 group-hover:border-slate-500/20';
  ...
  const itemBase = 'flex-shrink-0 flex items-center gap-2 px-2.5 h-6 rounded-md transition-all text-[11px] whitespace-nowrap';
  const itemHover = isDark ? 'hover:bg-[var(--theme-primary)]/20 hover:text-white' : 'hover:bg-black/5 hover:text-slate-900';
  const itemActive = 'bg-[var(--theme-primary)] text-white font-medium shadow-md';
  ...
  return (
    <div className="w-full max-w-[400px] mx-auto relative z-[70] transition-all duration-300">
      <form onSubmit={handleSearch} className="relative w-full group" ref={dropdownRef}>
        ...
        >
          ...
          <div className="relative h-full">
            <button
              ...
              className={`h-full flex items-center gap-2 pl-4 pr-3 rounded-l-2xl transition-colors min-w-[70px] ${dividerColor} ${isDark ? 'text-white/50 hover:text-white hover:bg-white/5' : 'text-slate-500 hover:text-slate-900 hover:bg-black/5'}`}
            >
              ...
              <ChevronDown size={14} className={`transition-transform duration-200 opacity-40 group-hover:opacity-100 ${isDropdownOpen ? 'rotate-180' : ''}`} />
            </button>
          </div>
          ...
          <input
            ...
            placeholder={t('search_placeholder', { engine: selectedEngine.name })}
          ...
          <button
            ...
            onBlur={() => setIsFocused(false)}
            type="submit"
            className={`p-2.5 mr-1 transition-all rounded-xl ${iconColor} ${isDark ? 'hover:bg-[var(--theme-primary)]/20 hover:text-white' : 'hover:bg-black/5 hover:text-slate-900'}`}
          >
            <Search size={18} />
          </button>
        </div>
        ...
        {/* Dropdown Menu */}
        {isDropdownOpen && (
              <div className={`absolute top-full left-0 right-0 mt-2 px-1 h-10 flex items-center ${dropdownClasses} rounded-xl overflow-hidden animate-fade-in origin-top z-[80] shadow-2xl`}>
                <div className="flex flex-row overflow-x-auto no-scrollbar gap-1.5 px-1 items-center w-full h-full">
                  {SEARCH_ENGINES.map((engine) => (
                    ...
                    >
                      <span className="w-3.5 h-3.5 flex items-center justify-center rounded-sm overflow-hidden shadow-sm bg-white/10">{engine.icon}</span>
